...
export const stepFlow = () => {
  ...
  const openActionModal = () => {
    ...
    gTl
      ...
      .to(backdrop, {
        opacity: 1,
        duration: 0.8,
        ease: "expo.out", // Smooth easing for backdrop opacity
      })
      .fromTo(
        actionWrapper,
        { x: "100%" }, // Start from off-screen to the right
        {
          x: 0,
          duration: 1,
          ease: "expo.out", // Smooth, fast-out easing
        },
        "-=0.7" // Overlapping with backdrop fade-in
      )
      .to(
        triggerCloseBtn,
        {
  ...
  const closeActionModal = () => {
    ...
    gTl
      .to(triggerCloseBtn, {
        opacity: 0,
        x: "-10vw", // Move close button slightly out before closing
        duration: 0.4,
        ease: "expo.out", // Smooth easing for fade-out
        scale: 0, // Shrink the close button slightly for a smooth effect
      })
      .to(
        actionWrapper,
        {
          ...
          duration: 0.8,
          ease: "expo.out", // Smooth, fast-in easing
        },
        "-=0.3" // Slight overlap with the close button animation
      )
      .to(
        backdrop,
        {
          opacity: 0,
          duration: 0.6,
          ease: "expo.out", // Smooth fade-out for backdrop
        },
        "-=0.7" // Overlap with the actionWrapper animation
      )
      .to(
        backdrop,
        {
